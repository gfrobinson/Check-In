...
const TZ_LABELS = {
  'America/Anchorage':'Alaska (AKST/AKDT)','America/Los_Angeles':'Pacific (PST/PDT)',
  'America/Denver':'Mountain (MST/MDT)','America/Chicago':'Central (CST/CDT)',
  'America/New_York':'Eastern (EST/EDT)','America/Halifax':'Atlantic (AST/ADT)',
  'Pacific/Honolulu':'Hawaii (HST)','Europe/London':'London (GMT/BST)',
  'Europe/Paris':'Central Europe (CET/CEST)','Asia/Tokyo':'Japan (JST)',
  'Asia/Shanghai':'China (CST)','Australia/Sydney':'Sydney (AEST/AEDT)',
};
    ...
    <nav style={{display:'flex',alignItems:'center',gap:'2rem',padding:'1rem 2rem',borderBottom:'1px solid #2a2d38',background:'#16181f',position:'sticky',top:0,zIndex:100}}>
      ...
      <div style={{display:'flex',gap:'1.5rem'}}>
        {['Dashboard','Check In','History','Settings'].map(l=>(
          <button key={l} onClick={()=>setView(l)} style={{background:'none',border:'none',cursor:'pointer',color:view===l?'#e8e9f0':'#8b8fa8',fontWeight:view===l?600:400,fontSize:'0.88rem',fontFamily:'inherit'}}>{l}</button>
        ))}
      </div>
      {user
...
function isDueToday(set) {
  ...
  if (set.frequency==='monthly') return date===parseInt(set.monthDay||1);
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.75rem'}}>
          {todoSets.map(s=>(
            <div key={s.id} style={{...S.surface,padding:'1.25rem 1.5rem',display:'flex',alignItems:'center',gap:'1rem'}}>
              ...
              <button style={S.btnPrimary} onClick={()=>{setActiveSetId(s.id);setView('Check In');}}>Start →</button>
            </div>
          ))}
          {doneSets.map(s=>(
...
function CheckInView({user,activeSetId,setActiveSetId,toast}) {
  ...
  useEffect(()=>{
    if (!user){setLoading(false);return;}
    Promise.all([getQuestionSets(user.uid),getUserProfile(user.uid)]).then(async([qs,prof])=>{
  ...
  if (!currentSet) return (
    <div style={{maxWidth:680,margin:'0 auto',padding:'2rem 1.5rem'}}>
      ...
      {sets.length===0
        ? <div style={{...S.surface,padding:'3rem',textAlign:'center',color:'#8b8fa8'}}>No Question Sets are due today.</div>
        : sets.map(s=>(
  ...
  if (submitted||todayDone) return (
    <div style={{maxWidth:680,margin:'4rem auto',textAlign:'center',padding:'0 1.5rem'}}>
      ...
      <button style={S.btnGhost} onClick={()=>{setCurrent(null);setActiveSetId(null);setSubmitted(false);}}>← Back to Check-Ins</button>
    </div>
  );
...
function HistoryView({user,activeSetId,setActiveSetId}) {
  ...
  async function loadCheckins(s){setLoading(true);const ci=await getCheckins(user.uid,s.id,500);setCheckins(ci);setLoading(false);}
  ...
  const filtered=range===0?checkins:checkins.filter(c=>{const cutoff=new Date();cutoff.setDate(cutoff.getDate()-range);return c.completedAt?.toDate()>=cutoff;});
  ...
  const avgScore = c => { const s=calcScore(c); return s===null?'—':s.toFixed(1); };
    ...
    <div style={{maxWidth:1100,margin:'0 auto',padding:'2rem 1.5rem'}}>
      ...
      {loading?<div style={{color:'#8b8fa8'}}>Loading…</div>:<>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'1.25rem',marginBottom:'1.25rem'}}>
          {(currentSet.questions||[]).filter(q=>q.type==='scale'||q.type==='yesno').map(q=>(
                ...
                <div>
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    {sorted.map((c,i)=>{
                      ...
                      const desired=q.desiredAnswer||'Yes';
                      const bg=a===undefined?'#2a2d38':a===desired?'rgba(52,211,153,0.75)':'rgba(239,68,68,0.65)';
                      return (
                        <div key={i} title={a!==undefined?`${date}: ${a}`:date}
                          style={{width:22,height:22,borderRadius:4,background:bg,cursor:'default',transition:'transform .1s'}}
                  ...
                  <div style={{display:'flex',gap:'1rem',marginTop:'0.75rem',fontSize:'0.78rem',color:'#8b8fa8'}}>
                    <span style={{display:'flex',alignItems:'center',gap:5}}><span style={{width:12,height:12,borderRadius:3,background:'rgba(52,211,153,0.75)',display:'inline-block'}}/> {q.desiredAnswer||'Yes'} (desired)</span>
                    <span style={{display:'flex',alignItems:'center',gap:5}}><span style={{width:12,height:12,borderRadius:3,background:'rgba(239,68,68,0.65)',display:'inline-block'}}/> {q.desiredAnswer==='Yes'?'No':'Yes'}</span>
                    <span style={{display:'flex',alignItems:'center',gap:5}}><span style={{width:12,height:12,borderRadius:3,background:'#2a2d38',display:'inline-block'}}/> Not answered</span>
                  </div>
                </div>
              )}
            </div>
...
function SettingsView({user,toast}) {
  ...
  const [profile,setProfile] = useState({timezone:'America/Anchorage',reminderEmail:'',alertFromEmail:'check-in@gfrobinson.com'});
  const [loading,setLoading] = useState(true);
  ...
  async function handleSaveSet(set){
    try{
      await saveQuestionSet(user.uid,set);
      const updated=await getQuestionSets(user.uid);
      setSets(updated);setEditing(null);
  ...
  async function handleDeleteSet(setId,name){
    ...
    try{await deleteQuestionSet(user.uid,setId);setSets(s=>s.filter(x=>x.id!==setId));toast('Question Set deleted.');}
    catch(e){toast(e.message,'error');}
  }

  if (!user) return <div style={{padding:'4rem',textAlign:'center',color:'#8b8fa8'}}>Please sign in to manage settings.</div>;
  if (loading) return <div style={{padding:'4rem',textAlign:'center',color:'#8b8fa8'}}>Loading…</div>;
  if (editing) return <SetEditor set={editing==='new'?null:editing} onSave={handleSaveSet} onCancel={()=>setEditing(null)} toast={toast}/>;
...
function SetEditor({set,onSave,onCancel,toast}) {
  ...
  const [acctEmail,setAcctEmail]     = useState(set?.accountabilityEmail||'');
  const [acctMode,setAcctMode]       = useState(set?.accountabilityMode||'summary');
  const dragItem = useRef(null);
  const dragOver = useRef(null);

  function addQuestion(type){setQuestions(qs=>[...qs,{id:genId(),type,text:'',options:type==='multiple_choice'?['Option A','Option B']:[],desiredAnswer:'Yes'}]);}
  ...
  function onDragEnd(){
    const qs=[...questions];
    const dragged=qs.splice(dragItem.current,1)[0];
    qs.splice(dragOver.current,0,dragged);
    dragItem.current=null;dragOver.current=null;
    setQuestions(qs);
  }

  // Arrow reorder
  function moveUp(i){if(i===0)return;const qs=[...questions];[qs[i-1],qs[i]]=[qs[i],qs[i-1]];setQuestions(qs);}
  function moveDown(i){if(i===questions.length-1)return;const qs=[...questions];[qs[i],qs[i+1]]=[qs[i+1],qs[i]];setQuestions(qs);}
  ...
  function handleSave(){
    ...
    onSave({id:set?.id,name,frequency,weekday,monthDay,customDays,reminderTime,questions,accountabilityEmail:acctEmail,accountabilityMode:acctMode});
  }

  return (
    <div style={{maxWidth:760,margin:'0 auto',padding:'2rem 1.5rem'}}>
      <button style={{...S.btnGhost,marginBottom:'1.5rem',fontSize:'0.82rem'}} onClick={onCancel}>← Back</button>
      <h1 style={{fontFamily:"'DM Serif Display',serif",fontSize:'2rem',marginBottom:'2rem'}}>{set?`Edit: ${set.name}`:'New Question Set'}</h1>
      ...
      <div style={{...S.surface,padding:'1.75rem',marginBottom:'1.25rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
          <div><label style={S.label}>Name</label><input style={S.input} placeholder="e.g. Daily Check-In" value={name} onChange={e=>setName(e.target.value)}/></div>
          <div><label style={S.label}>Reminder time (local time)</label><input style={S.input} type="time" value={reminderTime} onChange={e=>setReminderTime(e.target.value)}/></div>
          <div>
          ...
          {frequency==='weekly' && <div><label style={S.label}>Day of week</label><select style={S.input} value={weekday} onChange={e=>setWeekday(parseInt(e.target.value))}>{DAY_LABELS.map((d,i)=><option key={i} value={i}>{d}</option>)}</select></div>}
          {frequency==='monthly' && <div><label style={S.label}>Day of month</label><select style={S.input} value={monthDay} onChange={e=>setMonthDay(parseInt(e.target.value))}>{Array.from({length:28},(_,i)=>i+1).map(d=><option key={d} value={d}>{d}</option>)}</select></div>}
          {frequency==='custom' && (
            <div>
      ...
      <div style={{...S.surface,padding:'1.75rem',marginBottom:'1.25rem'}}>
        ...
        {questions.map((q,i)=>(
          <div key={q.id}
            ...
            style={{background:'#1e2028',border:'1px solid #2a2d38',borderRadius:10,padding:'1rem',marginBottom:'0.75rem',cursor:'grab'}}>
            <div style={{display:'flex',gap:'0.75rem',alignItems:'flex-start'}}>
              {/* Drag handle + arrows */}
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:2,paddingTop:4,minWidth:28}}>
                <span style={{color:'#8b8fa8',fontSize:'1rem',lineHeight:1,cursor:'grab',userSelect:'none'}}>⋮⋮</span>
                <button onClick={()=>moveUp(i)} disabled={i===0} style={{background:'none',border:'none',color:i===0?'#3a3d4a':'#8b8fa8',cursor:i===0?'default':'pointer',fontSize:'0.75rem',lineHeight:1,padding:'2px 0'}}>▲</button>
                <button onClick={()=>moveDown(i)} disabled={i===questions.length-1} style={{background:'none',border:'none',color:i===questions.length-1?'#3a3d4a':'#8b8fa8',cursor:i===questions.length-1?'default':'pointer',fontSize:'0.75rem',lineHeight:1,padding:'2px 0'}}>▼</button>
              </div>
              <div style={{flex:1,display:'flex',flexDirection:'column',gap:'0.5rem'}}>
                ...
                {q.type==='multiple_choice' && (
                  <div>
                    ...
                    <div style={{display:'flex',gap:'0.5rem'}}>
                      <input style={{...S.input,flex:1}} placeholder="New option…" id={`opt-${q.id}`} onKeyDown={e=>{if(e.key==='Enter'&&e.target.value.trim()){addOption(q.id,e.target.value.trim());e.target.value='';}}}/>
      ...
      <div style={{...S.surface,padding:'1.75rem',marginBottom:'1.25rem'}}>
        <div style={{fontFamily:"'DM Serif Display',serif",fontSize:'1.1rem',marginBottom:'0.5rem'}}>Accountability Partner</div>
        <p style={{color:'#8b8fa8',fontSize:'0.88rem',marginBottom:'1.25rem'}}>Optionally send results to someone after each check-in.</p>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
          <div>
            <label style={S.label}>Partner's email (optional)</label>
            <input style={S.input} type="email" placeholder="partner@example.com" value={acctEmail} onChange={e=>setAcctEmail(e.target.value)}/>
          </div>
          <div>
            <label style={S.label}>What to send</label>
            <select style={S.input} value={acctMode} onChange={e=>setAcctMode(e.target.value)}>
              <option value="summary">Summary score only</option>
